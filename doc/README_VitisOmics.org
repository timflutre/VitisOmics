...
*** Convert SNP data of the 18K Illumina chip from =xls= to =txt.gz=
On the command-line, working with tabulated files is much easier, and they should be compressed (e.g. with =gzip=).
The 18071 probe sequences also need to be saved in a fasta file.

See the corresponding task in the script =src/vitisomics.R=.

*** Map Illumina probes on several PN40024 assemblies
TODO
