...
Genomic sequences and annotations for grapevine are present at multiple locations, and it is not always easy to know precisely what the links are between the files available at these locations.
...
Files from NCBI (read first the README's):
#+begin_src sh
cd ncbi/
